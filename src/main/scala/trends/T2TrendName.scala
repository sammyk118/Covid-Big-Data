...
object TrendTwo {
  ...
  def createFiltered_country(mytable: DataFrame): DataFrame = {
    ...
    //calling function createSum_select returning all column names as Array of string
    val selected_column = createSum_select(mytable.columns, "temp")
    //getting  column names and data from temp and group by country/region.
    spark.sql(s"select $selected_column from temp where `Country/Region` in ($listofCountry) group by `Country/Region` ")

  }
 // return rtcolumn headers as "sum '1/10/2020'
 // drop all columns from left except 'Country/Region' and concatenate lt and rt column, and return comma separated string with column name
...
// saving data
 ...
 // return array of string as column_name with Temp_view
  ...
  // Separate date and left columns(provinces, Country/Region Lat, Long) using pattern matching as right columns and left columns
