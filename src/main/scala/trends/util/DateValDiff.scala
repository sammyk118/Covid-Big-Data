...
object DateValDiff {
    def divideDiffDF: DataFrame => DataFrame = df_handle => {
        joinRowToRow(divideDiff(df_handle.rdd -> df_handle.columns))
    }

    def divideDiffRDD: Tuple2[RDD[Row],Seq[String]] => Tuple2[RDD[Row],Seq[String]] = {
        case (rdd_handle, columns) =>
            val joinedDF = joinRowToRow(divideDiff(rdd_handle, columns))
            (joinedDF.rdd, joinedDF.columns)
    }
    
    def joinRowToRow: Tuple2[(DataFrame,Seq[String]),(DataFrame, Seq[String])] => DataFrame = {
    ...
    def diffRDD: Tuple2[RDD[Row],Seq[(String,String)]] => (DataFrame, Seq[String]) = {
        case (rdd_handle, diffCols) =>
            val unstructRDD = rdd_handle.map(row => Row(diffCols
                .foldLeft(Seq[Any]())((f, v) => f ++ Seq[Any](row.getAs[Any](v._1).toString.toLong - row.getAs[Long](v._2).toString.toLong)):_*))
